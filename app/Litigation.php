        ...
        if ($user->hasRole(['owner', 'admin'])) {
           ...
           // dd('aaaaa');
            ...
            if($country_of_origin > 0) {
                $litigations = DB::table('litigations as lg')
                    ->join('litigation_organization as lo', 'lo.litigation_id', '=', 'lg.id')
                    ->where('lo.organization_id', $user->organization_id)
                    ->where('lg.status','=',$status)
                    ->where('lg.country_of_origin','=',$country_of_origin)
                    ->distinct()->select('lg.*')
