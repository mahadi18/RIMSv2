...
//            ->where('litigations.status', 'closed')
    ...
    public static function getInitiatedCasesWithStatus($cId = 0, $status = '')
    {
        $initiatedCases = DB::table('litigations')
            ->select(
                DB::raw(
            ...
            ->leftJoin('users', 'users.id', '=', 'litigations.created_by_id')
            ->leftJoin('organizations', 'organizations.id', '=', 'users.organization_id')
            ->leftJoin('countries', 'countries.id', '=', 'organizations.country');
        if ($status) {
