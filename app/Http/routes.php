...
Route::get('/find/organizations/country', function(){
    
    $cid = $_GET['countryID'];

    $organizations = organizationsByCountry($cid);

    foreach ($organizations as $organization) {

        $orgatype = Organization::orgTypeName($organization->org_type);
        $district = Organization::orgDistrictName($organization->district_id);
        $country = Organization::orgCountryName($organization->country);
        
        echo '<option value="'.$organization->id.'">'.
                            $organization->name.','.
                            $orgatype.','.
                            $district.','.
                            $country.'</option>';
...
Route::get('/search/autocomplete', function(){
    ...
    $queries = DB::table('litigations')
        ...
        //return Auth::user()->organization()->get()[0]->name;
        //return Auth::user()->organization()->get()[0]->id;

        /*    $cnt=0;*/
        
        foreach ($queries as $query)
        { 
            //dump($query->id." => user_ID = ".$query->created_by_id);
            
            $created_by = DB::table('users')
                                    ->where('id', '=', $query->created_by_id)
                                    ->first();

            if(isset($created_by->organization_id) && $created_by->organization_id == Auth::user()->organization()->get()[0]->id )
            {
                $results[] = [ 'id' => $query->id, 'value' => 
                    $query->name_during_rescue.
                    (($query->rescue_date!= '') ? ' ( Rescue Date : ' : '' ).$query->rescue_date.(($query->rescue_date!= '') ? ' ) ' : '' ).
                    (($query->rescued_from_address!= '') ? ' ( Rescue Address : ' : '' ).str_limit($query->rescued_from_address, 30).(($query->rescued_from_address!= '') ? ' ) ' : '' ) ];
            }
        }

        //die;

        //autocomplete style backup
        /*'<div class="suggestions"><strong style="color:#1E9D8A">'.$query->name_during_rescue.'</strong>'.(($query->rescue_date!= '') ? '<br><b>Rescue Date</b>: ' : '' ).$query->rescue_date.(($query->rescued_from_address!= '') ? '<br><b>Rescue Address</b>: ' : '' ).str_limit($query->rescued_from_address, 30).'</div>'*/ 
...
Route::get('/test', function(){
    return loggedin_users_organization();
});

Route::get('home', 'HomeController@index');
Route::get('unauthorized', 'HomeController@index');
...
Route::get('activate-deactivate/{id}', "UserController@ActivateOrDeactivate");
...
Route::get('cases/{id}/save-take-over', ['uses' => 'LitigationController@saveTakeOverGet']);
...
/*Route::get('cases/{id}/accessibility', ['as' => 'get.accessibility', 'uses' => 'LitigationController@NGOofSelectedCountry']);*/
...
Route::resource("/email/create","MessageController@createEmail");
Route::resource("/email/send","MessageController@sendEmail");
