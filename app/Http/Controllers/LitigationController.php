...
class LitigationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //echo "request=".$request['status'].", Status=".$status;
        //die();
        ...
        if(Auth::user()->roles[0]->name=='owner' || get_organization_country(Auth::user()->organization_id)==2) {
            ...
            //echo "request=".$request['country_of_origin'].", country_of_origin=".$country_of_origin;
            //die();
        }
        else {
            $country_of_origin = 0;  
        ...
        if(Auth::user()->roles[0]->name=='contributor'){
            $litigations = Litigation::getAssignedLitigationsWithTasks($logged_organization,$status,10,$country_of_origin);
        }

        else {
            $litigations = Litigation::allLitigationsWithTasks($status,10,$country_of_origin);
        }
        ...
        foreach($litigations as $litigation){
            ...
            $physical = Ngohir::where('litigation_id', '=', $litigation->id)->first();

            //dd($physical->country);

            if(isset($physical->nationality) )
            {
                if($physical->nationality == "1")
                    $physical->nationality = "Bangladesh";
                elseif($physical->nationality == "3")
                    $physical->nationality = "Nepal";
                $litigation->country = $physical->nationality;
            }
            else
            {
                $litigation->country = Country::getCountryNameFromID($litigation->nationality);
                /*var_dump($litigation->country);
                echo " litigation er <br>";*/
            }
            ...
            $created_by = User::where('id', '=', $litigation->created_by_id)->first();
            $litigation->created_by_name = $created_by->name;
            //$litigation->created_by_id = $created_by->name;
    ...
    public function update(LitigationRequest $request, $id)
    {
        ...
        if($today < $date_of_birth)
        {
            return back()->with('error', 'Date is invalid!! Please enter a valid date.');;
        }
        $litigation->name_during_rescue                 = $request->input("name_during_rescue");
        $litigation->sex                                = $request->input("sex");
...
//        return redirect()->route('cases.edit', $id)->with('message', 'Personal Information updated successfully.');
    ...
    public function destroy(Request $request,$id)
    {
        ...
        $addresses = DB::table('addresses')
                    ...
                    ->delete();
        //2
        $attachments = DB::table('attachment_litigation')
                    ->where('litigation_id', '=', $litigation->id)
                    ->delete();
        //3
        $cares = DB::table('cares')
                    ->where('litigation_id', '=', $litigation->id)
                    ->delete();
        //4
        $care_plan_litigation = DB::table('care_plan_litigation')
                    ->where('litigation_id', '=', $litigation->id)
                    ->delete();
        //5
        $case_profiles = DB::table('case_profiles')
                    ->where('litigation_id', '=', $litigation->id)
                    ->delete();
        //6
        $case_studies = DB::table('case_studies')
                    ->where('litigation_id', '=', $litigation->id)
                    ->delete();
        //7
        ...
        $family_members = DB::table('family_members')
                    ...
                    ->delete();
        //9
        $field_litigation = DB::table('field_litigation')
                    ->where('litigation_id', '=', $litigation->id)
                    ->delete();
        //10
        $litigation_form = DB::table('litigation_form')
                    ->where('litigation_id', '=', $litigation->id)
                    ->delete();
        //11
        $litigation_organization = DB::table('litigation_organization')
                    ->where('litigation_id', '=', $litigation->id)
                    ->delete();
        //12
        $litigation_task_task_status = DB::table('litigation_task_task_status')
                    ->where('litigation_id', '=', $litigation->id)
                    ->delete();
        //13
        ...
        $movements = DB::table('movements')
                    ...
                    ->delete();
        //15
        $ngohirfiles = DB::table('ngohirfiles')
                    ->where('litigation_id', '=', $litigation->id)
                    ->delete();
        //16
        $ngohirs = DB::table('ngohirs')
                    ->where('litigation_id', '=', $litigation->id)
                    ->delete();
        //17
        $proceedings = DB::table('proceedings')
                    ->where('litigation_id', '=', $litigation->id)
                    ->delete();
        //18
        $takeovers = DB::table('takeovers')
                    ->where('litigation_id', '=', $litigation->id)
                    ->delete();
        ...
        return redirect()->route('cases.index')->with('message', 'Case deleted successfully.');
        //return back();
    }
...
//        return redirect()->route('cases.edit', $id)->with('message', 'Personal Information updated successfully.');
    ...
    public function showCaseProfile($litigation_id)
    {
        ...
        $role = DB::table('role_user')
            ...
            return view('litigations.case_profile', compact('litigation', 'histories', 'role'));
    ...
    public function fullProfile($litigation_id)
    {
		...
		$present_address = Address::where('litigation_id', '=', $litigation_id)
                        ->where('title', '=', 'present_address')
                        ->first();
        $native_address = Address::where('litigation_id', '=', $litigation_id)
                        ->where('title', '=', 'native_address')
                        ->first();
        ...
        $information = array();
        $sub_tasks = Litigation::getAccessbileTasks($litigation_id,$parent_task_id);
        //dd($sub_tasks);
        
            $task_id_to_show = $parent_task_id;
        ...
        //dd($task_id_to_show);
        $information = Litigation::getCaseInfoWithTasks($template_task,$litigation_id);
                
        return view('litigations.full_profile', compact('information','litigation', 'physical', 'present_address', 'native_address'));
    ...
    public function saveTakeOver(Request $request=null, $lid=null)
    {
        ...
        return redirect('cases/'.$lid.'/take-over')->with('message','Takeover Information Updated Successfully');
    }

    public function saveTakeOverGet(Request $request=null, $lid=null)
    {
        return redirect('cases/'.$lid.'/take-over')->with('message','Takeover Information Updated Successfully');
    }
    ...
    {
        $organizations = Organization::orderByRaw('country')->get();
    ...
    public function saveAccessibility(Request $request, $lid)
    {
        ...
        /*return $request->input('organization');
        die;*/
