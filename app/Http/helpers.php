    ...
    function header_link()
    {
        ...
        switch ($user->roles[0]->name) {
            case 'contributor':
                ...
                $link = '<a style="font-size:12px; background-color:#E9FFFA; padding:9px 4px !important; border-radius:4;color:#1E9D8A; border:1px solid #1E9D8A;" href="/organization/dashboard"><i class="fa fa-align-justify"></i> Cases</a>';
                break;
...
if (!function_exists('get_organizations_list')) {
    ...
    {
        $organizations = \App\Organization::where('id', '>', 1)->lists('name', 'id')->all();

        array_unshift($organizations, "Please Select");
...
if (!function_exists('get_countries_list')) {
    ...
    {
        $countries = \App\Country::lists('name', 'id')->all();
...
if (!function_exists('states_of_country')) {
    ...
    function states_of_country($country_id)
    {
        ...
        $states = \App\State::where('country_id', '=', $country_id)
                                ->lists('name', 'id')
                                ->all();
...
if (!function_exists('get_attachments_of_victim')) {
    ...
    function get_latest_attachments_of_victim($doc_name, $litigation_id)
    {
        $attachments = \App\Litigation::find($litigation_id)
            ->attachments()
            ->where('doc_type_id', get_doc_type_id($doc_name))
            ->orderBy('updated_at', 'desc')->offset(0)->limit(1)->get();;
        ...
        if ($count > 0) {
            foreach ($items as &$item) {
                // print_r($item);
                $dom .= '<img src="' . $item['file_path'] . '" alt="victim">';
            }
        } else {
            $dom .= '<img src=/uploads/-text.png alt="victim\'s image">';
        }
        //dd($dom);
...
if (!function_exists('organizationByCountry')) {
    function organizationsByCountry($cid = 0)
    {
        $OrganizationsByCountry = DB::table('organizations')
            ->where('organizations.country', $cid)
...
if (!function_exists('getCountryOfOrigin')) {
    function getCountryOfOrigin()
    {
        $countryOfOrigin = DB::table('litigations')
            ...
            ->leftJoin('countries', 'countries.id', '=', 'litigations.nationality')
            ->groupBy('litigations.nationality')
            ->orderBy('countries.name')
            ->get();
            //->toSql();
        //echo $countryOfOrigin;
        //die();
...
if (!function_exists('country_from_user_id')) {
    function country_from_user_id($user_id)
    {
        ...
        $country = DB::table('users')->select('countries.name')
            ...
            //dd($country);
        if($user_id==0)  // add by Ripon for eliminate pagination error (Temporary solution)
        return 'India';  // add by Ripon for eliminate pagination error (Temporary solution)
        else
        return $country->name;
    }
}
