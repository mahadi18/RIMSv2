...
<section class="panel">
    <div class="panel-body">
        <div class="text-center mbot30">
            ...
            <?php if($litigation->status=='open') { ?>
                <form action="/cases/{{$litigation->id}}/change_status" method="post" style="display: inline;">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                ...
                @if( $role->role_id == 1 )
                <form action="{{ route('cases.destroy', $litigation->id) }}" method="POST" style="display: inline;" onsubmit="if(confirm('Do you want to delete this case history?')) { return true } else {return false };"><input type="hidden" name="_method" value="DELETE"><input type="hidden" name="_token" value="{{ csrf_token() }}"> 
